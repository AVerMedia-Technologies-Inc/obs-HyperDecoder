...
{
	static bool print_first_video = false;
	...
	if (!ffmpeg_decode_valid(video_decoder)) {
		...
		print_first_video = false; // reset flag
	}
	...
	if (got_output) {
		...
		if (print_first_video == false)
		blog(LOG_INFO, "video ts: %llu", frame.timestamp);
		print_first_video = true;
#endif
...
static bool contains_07ca(const char* str) {
  // Use strstr to find the first occurrence of "07ca" in str
  const char* substring = strstr(str, "vid_07ca");
  return substring != nullptr;
}

static bool contains_1515(const char* str) {
  // Use strstr to find the first occurrence of "1515" in str
  const char* substring = strstr(str, "pid_1515");
  return substring != nullptr;
}
...
static obs_properties_t *GetDShowProperties(void *obj)
{
	...
	Device::EnumVideoDevices(data->devices);
	char* name_utf8 = nullptr;
	char* path_utf8 = nullptr;
	for (const VideoDevice &device : data->devices) {
		os_wcs_to_utf8_ptr(device.name.c_str(), device.name.size(),
				   &name_utf8);
		os_wcs_to_utf8_ptr(device.path.c_str(), device.path.size(),
				   &path_utf8);
		//blog(LOG_INFO, "%s: %s", name_utf8, path_utf8);
		if (contains_07ca(path_utf8) && contains_1515(path_utf8)) {
			AddDevice(p, device);
		}
		bfree(name_utf8);
		bfree(path_utf8);
	...
	obs_property_set_visible(p, false); // hide it for now
