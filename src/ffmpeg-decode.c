...
//#define ENABLE_AUDIO_COPY_DATA 1
//#define ENABLE_VIDEO_COPY_DATA 1
		       ...
		       bool use_hw)
{
	...
	if (AV_CODEC_ID_H264 == id) { // onlt apply to H.264
		// https://www.cnblogs.com/TaigaCon/p/10220356.html
		//decode->decoder->thread_count = 0;
		blog(LOG_INFO, "[ffmpeg_decode_init] %s", decode->codec->name);
		decode->decoder->thread_type = FF_THREAD_SLICE; // FF_THREAD_FRAME FF_THREAD_SLICE
		decode->decoder->thread_count = 4; // depending on cpu?
		blog(LOG_INFO, "[ffmpeg_decode_init] type=%d, count=%d",
				decode->decoder->thread_type, decode->decoder->thread_count);
	}
...
#ifdef ENABLE_AUDIO_COPY_DATA
	copy_data(decode, data, size);
#endif // ENABLE_AUDIO_COPY_DATA
			 ...
			 struct obs_source_frame2 *frame, bool *got_output)
{
	...
	AVFrame *out_frame;
	int ret;
	static bool print_first_iframe = false;

	*got_output = false;

#ifdef ENABLE_VIDEO_COPY_DATA
	copy_data(decode, data, size);
#endif // ENABLE_VIDEO_COPY_DATA
	...
	if (!decode->frame) {
		...
		print_first_iframe = false; // reset flag
	}
	...
	case AV_CODEC_ID_H264:
		if (obs_avc_keyframe(data, size)) {
			...
			if (print_first_iframe == false)
				blog(LOG_INFO, "obs_avc_keyframe: %llu", packet->pts);
			print_first_iframe = true;
		}
#ifdef ENABLE_HEVC
		break;
